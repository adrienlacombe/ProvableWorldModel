...
//!
//! This module owns the canonical observability types (`docs/spec/05-observability.md`):
//!
//! - [`record`] — the [`LogRecord`] schema, the [`Level`] policy, and the
//!   per-stage required-field contract.
//! - [`value`] — [`LoggableValue`] and the redaction guard. Redaction is a
//!   type-level guarantee (INV-OBS-02): a witness value is not *representable* as
//!   a loggable field, so a careless log call is a compile error, not a leak.
//! - [`metric`] — the named metric set, label domains, and the sample/snapshot
//!   wire types.
//! - [`span`] — the normative span names and the canonical
