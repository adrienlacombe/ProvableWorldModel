...
| Build toolchain (V0) | a single pinned nightly recorded in `rust-toolchain.toml` (`nightly-2025-07-14`) | The vendored proving substrate (Stwo `v2.2.0`, stwo-circuits `v0.1.0`) requires nightly Rust; there is no stable MSRV for the workspace during V0 (see the resolution below). |
| Toolchain policy | The pinned nightly may only rise on a `MINOR` (`0.x`) / `MAJOR` (`>=1.0`) release; never on a `PATCH`. Every bump is a `Changed` changelog entry. | A toolchain-floor rise can break downstream builds and is therefore a non-`PATCH` event. |
| Toolchain pinning | `rust-toolchain.toml` at the workspace root pins the exact nightly used for reproducible builds; CI runs every gate on that pinned nightly. | The reproducibility contract (RFC-0016) requires a known toolchain. |

RESOLVED (2026-06-03, [RFC-0015 toolchain decision](../rfcs/RFC-0015-third-party-vendoring-and-pinning.md#toolchain-decision-2026-06-03-amendment)): the vendored revisions are frozen (`stwo` `v2.2.0` `289c20de`, `stwo-circuits` `v0.1.0` `b0db13e4`). Both require nightly Rust and use nightly-only features, so the "lowest stable Rust" MSRV model above is **superseded for the V0 line**: the project pins `nightly-2025-07-14` in `rust-toolchain.toml`. There is no stable MSRV during V0; the concept returns if the proving substrate later gains a stable-Rust path. Because `pwm-verifier` depends on the vendored Stwo verifier path, downstream verifier integrators also build on the pinned nightly for V0 — a cost forced by the upstream dependency, recorded in the RFC-0015 amendment.
